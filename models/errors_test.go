...
func TestIsUniqueViolation(t *testing.T) {
	// Assert: nil-ошибка не считается unique violation.
	if IsUniqueViolation(nil) {
		t.Fatal("IsUniqueViolation(nil) = true, want false")
	}
	// Assert: произвольная ошибка без PG-кода 23505 — false.
...
func TestMonitorURLUniqueConstraint(t *testing.T) {
	// Arrange: чистая БД для проверки уникальности URL на уровне PostgreSQL.
	db := openTestDB(t)
